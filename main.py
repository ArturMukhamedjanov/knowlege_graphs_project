...
if ontology_graph:
    ...
    #Агент с лучшим винрейтом на карте Bind с другим агентом и наличием у обоих по 2 лайнапа
    ...
    WHERE {
        ...
        # Убедимся, что оба агента имеют не менее 2 лайнапов на карте Bind
        ?agent ex:hasSkill ?skill1 .
        ex:Bind ex:hasLineup ?skill1 .
        
        ?withAgent ex:hasSkill ?skill2 .
        ex:Bind ex:hasLineup ?skill2 .
        
        # Считаем количество уникальных лайнапов для каждого агента
        {
            SELECT ?agent (COUNT(?skill1) AS ?skillCount1)
            WHERE {
                ?agent ex:hasSkill ?skill1 .
                ex:Bind ex:hasLineup ?skill1 .
            }
            GROUP BY ?agent
            HAVING (?skillCount1 >= 2)
        }
        
        {
            SELECT ?withAgent (COUNT(?skill2) AS ?skillCount2)
            WHERE {
                ?withAgent ex:hasSkill ?skill2 .
                ex:Bind ex:hasLineup ?skill2 .
            }
            GROUP BY ?withAgent
            HAVING (?skillCount2 >= 2)
        }
    }
    ORDER BY DESC(?winrate)
    LIMIT 1
